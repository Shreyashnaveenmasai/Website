...
const Contact = () => {
    return (
        <Container maxWidth="lg" sx={{ mt: 10, mb: 10, backgroundColor: colors.background, p: 5, borderRadius: 2, boxShadow: 3 }}>
            {/* Contact Details */}
            <Grid container spacing={6} justifyContent="center" component={motion.div} variants={staggerContainer} initial="hidden" animate="visible">
                {[
                    {
                        icon: <RoomIcon fontSize="large" sx={{ color: colors.accent }} />,
                        title: "OUR ADDRESS",
                        text: "16220 N Scottsdale Road, Scottsdale, Arizona 85254-USA"
                    },
                    {
                        icon: <EmailIcon fontSize="large" sx={{ color: colors.accent }} />,
                        title: "MESSAGE US",
                    ...
                    <Grid item xs={12} sm={6} md={4} key={index} component={motion.div} variants={fadeInUp}>
                        <motion.div whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.95 }}>
                            <Card sx={{ textAlign: "center", p: 3, boxShadow: 4, backgroundColor: "white" }}>
                                <CardContent>
                                    {item.icon}
                                    <Typography variant="h6" fontWeight="bold" sx={{ mt: 1, color: colors.primary }}>
                                        {item.title}
                                    </Typography>
            ...
            <Grid container spacing={4} sx={{ mt: 5 }} alignItems="center">
                ...
                <Grid item xs={12} md={6}>
                    <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, ease: "easeOut" }}>
                        <Typography variant="h5" fontWeight="bold" sx={{ color: colors.primary }} gutterBottom>
                            CONTACT FORM
                        </Typography>
                        <Grid container spacing={2} component={motion.div} variants={staggerContainer} initial="hidden" animate="visible">
                            {["Name", "Email", "Phone", "Subject"].map((label, index) => (
                                <Grid item xs={12} sm={6} key={index} component={motion.div} variants={fadeInUp}>
                                    <TextField fullWidth label={label} variant="outlined" sx={{
                                        mb: 2,
                            ...
                            <Grid item xs={12} component={motion.div} variants={fadeInUp}>
                                <TextField fullWidth label="Your Message" variant="outlined" multiline rows={4} sx={{
                                    mb: 2,
                                    "& .MuiOutlinedInput-root": { borderRadius: 2 },
                                    "& label.Mui-focused": { color: colors.primary },
                                    "& .MuiOutlinedInput-notchedOutline": { borderColor: colors.primary }
